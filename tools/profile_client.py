...
def _interactive_live(args: argparse.Namespace) -> None:
    ...
    sample_limit = (
        ...
        else _ask_non_negative_int("Sample limit (0 = unlimited)", 0)
    )
    max_hops = (
        args.max_hops
        if args.max_hops is not None
        else _ask_non_negative_int("Max schema path hops", 3)
    )
    value_limit = (
        args.value_limit
        if args.value_limit is not None
        else _ask_non_negative_int("Value profile limit (0 = unlimited)", 0)
    )
    include_values = (
        args.include_values
        if args.include_values is not None
        else _ask_yes_no("Include value profile?", True)
    )
...
def _interactive_csv(args: argparse.Namespace) -> None:
    ...
    csv_path = Path(
        _ask("CSV path", args.csv_path or "D:\\Agent\\t2c_eval_framework\\inputs\\northwind.csv")
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Interactive client for building Text-to-Cypher profile JSON files."
    )
    parser.add_argument(
        "--mode", choices=("live", "csv"), default="live", help="Profile source mode."
    )
    ...
    parser.add_argument(
        "--profile-name",
        default="",
        help="Logical profile name when it differs from the physical Neo4j database.",
    )
    ...
    parser.add_argument(
        "--sample-limit", type=int, default=None, help="Live sampling limit. 0 means unlimited."
    )
    parser.add_argument(
        "--max-hops",
        type=int,
    ...
    value_group.add_argument(
        ...
        action="store_true",
        default=None,
        help="Include live value profile.",
    )
    value_group.add_argument(
        "--no-values", dest="include_values", action="store_false", help="Skip live value profile."
    )
    parser.add_argument("--preview", type=int, default=5, help="Preview rows to print.")
    parser.add_argument(
        "--force", action="store_true", help="Overwrite an existing profile without asking."
    )
