...
def parse_schema_symbols(schema_text: str) -> tuple[set[str], set[str]]:
    """Extract labels and relationship types from schema patterns."""
    labels: set[str] = set()
    relationships: set[str] = set()
    for line in schema_text.splitlines():
        labels.update(re.findall(r"\(:([A-Za-z0-9_]+)\)", line))
        relationships.update(re.findall(r"\[:([A-Za-z0-9_]+)\]", line))
